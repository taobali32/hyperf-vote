...
use Jtar\HyperfFollow\Traits\Followable;
...
trait Follower
{
    public function follow(Model $followable): array
    {
        ...
        /** @var Model|\Jtar\HyperfFollow\Traits\Followable $followable */
        ...
        $this->followings()->updateOrCreate([
            ...
            'accepted_at' => $isPending ? null : date('y-m-d H:i:s')
        ]);
    ...
    {
        if (!in_array(\Jtar\HyperfFollow\Traits\Followable::class, \class_uses($followable))) {
            throw new InvalidArgumentException('The followable model must use the Followable trait.');
        }
